...
- **REST APIs swap the host** (`liveshopping-api.bambuser.com` → `liveshopping-api-eu.bambuser.com`), and an API key is only valid in its own region — the wrong base URL fails auth rather than returning data (see [`rest-apis.md`](rest-apis.md)).
